...
public class FilterGroovyScriptImpl implements Filter {
    ...
    public void doFilter(IMessage msg, Map params) throws Exception {
        ...
        if (StringUtils.isNoneEmpty(filterConfigGroovyScript.getGroovyScript())) {
            Object value = shell.evaluate(filterConfigGroovyScript.getGroovyScript());
        }
        if (StringUtils.isNotEmpty(filterConfigGroovyScript.getGroovyScriptFilePath())) {
            String script = new String(Files.readAllBytes(Paths.get(filterConfigGroovyScript.getGroovyScriptFilePath())));
