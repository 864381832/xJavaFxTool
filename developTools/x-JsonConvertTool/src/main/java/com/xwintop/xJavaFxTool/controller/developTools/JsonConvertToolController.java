...
public class JsonConvertToolController extends JsonConvertToolView {
    ...
    @FXML
    private void exchangeValueAction(ActionEvent event) throws Exception {
        String jsonTextAreaString = jsonTextArea.getText();
        jsonTextArea.setText(afterTextArea.getText());
        afterTextArea.setText(jsonTextAreaString);
    }
    ...
    @FXML
    private void jsonToJsonPathAction(ActionEvent event) throws Exception {
        try {
            jsonConvertToolService.jsonToJsonPathAction();
        } catch (Exception e) {
            log.error("转换错误：", e);
            TooltipUtil.showToast("转换错误" + e.getMessage());
        }
