...
//            lineChart.setCreateSymbols(false);
    ...
    private String getSystemInfoString() {
        ...
        stringBuffer.append(printPowerSources(hal.getPowerSources().toArray(new PowerSource[0])));

        //LOG.info("Checking Disks...");
        stringBuffer.append(printDisks(hal.getDiskStores().toArray(new HWDiskStore[0])));

        //LOG.info("Checking File System...");
        stringBuffer.append(printFileSystem(os.getFileSystem()));

        //LOG.info("Checking Network interfaces...");
        stringBuffer.append(printNetworkInterfaces(hal.getNetworkIFs().toArray(new NetworkIF[0])));
        ...
        stringBuffer.append(printDisplays(hal.getDisplays().toArray(new Display[0])));

        // hardware: USB devices
        //LOG.info("Checking USB Devices...");
        stringBuffer.append(printUsbDevices(hal.getUsbDevices(true).toArray(new UsbDevice[0])));

        //LOG.info("Checking Sound Cards...");
        stringBuffer.append(printSoundCards(hal.getSoundCards().toArray(new SoundCard[0])));
    ...
    private static String printMemory(GlobalMemory memory) {
        ...
        stringBuffer.append("\nSwap used: " + FormatUtil.formatBytes(memory.getVirtualMemory().getSwapUsed()) + "/" + FormatUtil.formatBytes(memory.getVirtualMemory().getSwapTotal()));
        return stringBuffer.toString();
    }
...
//        stringBuffer.append(String.format("CPU load: %.1f%% (OS MXBean)%n", processor.getSystemCpuLoad() * 100));
        stringBuffer.append(String.format("CPU load: %.1f%% (counting ticks)%n", processor.getSystemCpuLoadBetweenTicks(processor.getSystemCpuLoadTicks()) * 100));
        stringBuffer.append(String.format("CPU load: %.1f%% (OS MXBean)%n", processor.getSystemCpuLoadTicks()[0] * 100F));
...
//        stringBuffer.append("\n Fan Speeds: " + Arrays.toString(sensors.getFanSpeeds()));
    ...
    private static String printPowerSources(PowerSource[] powerSources) {
        ...
        if (powerSources.length == 0) {
            ...
            double timeRemaining = powerSources[0].getTimeRemainingInstant();
            if (timeRemaining < -1d) {
                sb.append("Charging");
