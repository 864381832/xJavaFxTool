...
public class SocketServerHandler extends IoHandlerAdapter {
    ...
    public void messageReceived(IoSession session, Object message) throws Exception {
        ...
        if (socketToolService.getSocketToolController().getEnableAutoReplyCheckBox().isSelected()) {
            if (socketToolService.getSocketToolController().getAutoReplyTypeChoiceBox().getSelectionModel().getSelectedIndex() == 0) {
                socketToolService.serverDataSendAction(message.toString());
            } else {
                socketToolService.serverDataSendAction(socketToolService.getSocketToolController().getAutoReplyTextField().getText());
            }
        }
    }
