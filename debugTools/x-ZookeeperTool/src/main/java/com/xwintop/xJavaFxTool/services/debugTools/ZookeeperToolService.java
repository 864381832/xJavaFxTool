...
public class ZookeeperToolService {
    ...
    public void connectOnAction() {
        ...
        zookeeperToolController.getConnectButton().setDisable(true);
        zookeeperToolController.getDisconnectButton().setDisable(false);
        zookeeperToolController.getRefreshButton().setDisable(false);
    }
    ...
    private void addNodeTree(String path, TreeItem<String> treeItem) {
        try {
            ...
            for (String name : list) {
                ...
                try {
                    List<String> listChildren = zkClient.getChildren(StringUtils.appendIfMissing(path, "/", "/") + name);
                    for (String nameChildren : listChildren) {
                        treeItem2.getChildren().add(new TreeItem<>(nameChildren));
                    }
                } catch (Exception e) {
                    log.error("加载节点失败：", e);
    ...
    public void disconnectOnAction() {
        ...
        zookeeperToolController.getNodeTreeView().getRoot().getChildren().clear();
        zookeeperToolController.getConnectButton().setDisable(false);
        zookeeperToolController.getDisconnectButton().setDisable(true);
        zookeeperToolController.getRefreshButton().setDisable(true);
    }
    ...
    public void addNodeOnAction() {
        ...
        String nodeName = AlertUtil.showInputAlertDefaultValue("请输入节点名称：", "");
    ...
    public void renameNodeOnAction(boolean isCopy) {
        ...
        String nodeName = AlertUtil.showInputAlertDefaultValue("请输入节点新名称：", "");
        if (StringUtils.isEmpty(nodeName)) {
            TooltipUtil.showToast("节点名不能为空！");
            return;
        }
